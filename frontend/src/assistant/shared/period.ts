...
const normalizePhrase = (value: string): string => {
  ...
  phrase = phrase.replace(/^(los|las|the)\s+de\s+/, "")
  phrase = phrase.replace(/^(el|la|the)\s+/, "")
  return phrase.replace(/\s+/g, "_")
}

const PHRASE_TO_PERIOD: Array<{ pattern: RegExp; key: string }> = [
  { pattern: /^(hoy|today)$/, key: "hoy" },
  { pattern: /^(ayer|yesterday)$/, key: "ayer" },
  { pattern: /^(manana|mañana|tomorrow)$/, key: "manana" },
  { pattern: /^(esta_semana|this_week)$/, key: "esta_semana" },
  { pattern: /^(semana_pasada|last_week)$/, key: "semana_pasada" },
  { pattern: /^(este_mes|this_month)$/, key: "este_mes" },
  { pattern: /^(mes_pasado|last_month)$/, key: "mes_pasado" },
  { pattern: /^(este|el)_ano$/, key: "este_ano" },
  { pattern: /^this_year$/, key: "este_ano" },
  { pattern: /^(ano_pasado|last_year)$/, key: "ano_pasado" },
]
