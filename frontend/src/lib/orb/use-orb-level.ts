...
import { approach, stateEnergy, type OrbState } from "@/lib/orb/orb-state"
...
export const useOrbLevel = (
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      const target = hasLive ? live : stateEnergy(stateRef.current, clockRef.current)
